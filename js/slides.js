...
export function initDepthBasedNVS() {
  ...
  // Returns the offline-rendered novel view as a canvas, or null if the asset
  // is missing. Generated by scripts/render_novel_view.py — same depth model
  // and warp parameters as buildTargetWarp, then LaMa-inpainted.
  async function loadPrerenderedRefined() {
    try {
      return await loadImageCanvas("assets/images/redtoyota_novel_view.jpg");
    } catch {
      return null;
    }
  }
  ...
  function ensureDepthArtifacts() {
    ...
    depthPromise = (async () => {
      ...
      refinedCanvas =
        (await loadPrerenderedRefined()) ||
        buildRefinedResult(warpCanvas, holeMap);
      setLoading("", false);
      if (active > 0) {
