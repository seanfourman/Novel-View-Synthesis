...
export function loadDepthPipeline(progressCallback) {
  ...
  pipePromise = (async () => {
    ...
    progressCallback?.({ status: 'init', text: 'מאתחל מודל...', progress: 0.02 });
        ...
        const p = await pipeline('depth-estimation', MODEL_ID, {
          ...
          progress_callback: (info) => {
            if (info?.status === 'progress' && info?.file) {
              ...
              progressCallback?.({ status: 'ready', text: 'מודל טעון', progress: 0.85 });
            }
          },
        });
        modelLoaded = true;
        progressCallback?.({ status: 'loaded', text: 'מודל מוכן', progress: 0.9 });
        return p;
      } catch (err) {
        console.warn(`depth pipeline failed with ${JSON.stringify(opts)}:`, err);
        lastErr = err;
      }
    }
...
export async function estimateDepth(imageCanvas, progressCallback) {
  ...
  progressCallback?.({ status: 'infer', text: 'מעריך עומק...', progress: 0.92 });
